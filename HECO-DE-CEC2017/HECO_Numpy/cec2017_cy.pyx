...
cdef class CFunction:
    cdef tuple _load_mat(self, int benchID, Py_ssize_t D): #load .mat files for python
        cdef:
            double[:, ::1] o
            double[:, :] M, M1, M2
    ...
    def load_mat(self, benchID, D):
        return self._load_mat(benchID, D)
    
    cdef tuple benchmark(self, int benchID, Py_ssize_t D, double[::1] x, double[:, ::1] o, double[:, :] M, double[:, :] M1, double[:, :] M2):
        cdef:
            double[::1] z, y, w, absZ, g, h 
            double f, f0, f1, g0, g1, g2, h0, h1, h2, h3, h4, h5, temp, temp0, temp1
            int len_g, len_h
            Py_ssize_t i, j
...
#        z, y, w, absZ = (np.zeros(D, dtype = np.float64) for _ in range(4))
#        g, h = (np.zeros(10, dtype = np.float64) for _ in range(2))
        f, f0, f1, g0, g1, g2, h0, h1, h2, h3, h4, h5, c1, c2, c3, v = (0.0 for _ in range(16))
        len_g, len_h = 0, 0
...
#            
    ...
    def get_LB(self, double[::1] lb, int benchID, Py_ssize_t D): #get lower bound of solutions
        if benchID in range(1, 4) or [8] or range(10, 19) or range(20, 28):
            ...
            for i in range(D):
                lb[i] = -50.0
    def get_UB(self, double[::1] ub, int benchID, Py_ssize_t D): #get upper bound of solutions
        if benchID in range(1, 4) or [8] or range(10, 19) or range(20, 28):
            for i in range(D):
                ub[i] = 100.0 
        elif benchID in [4, 5, 9]:
    ...
    cdef void _evaluate(self, int benchID, Py_ssize_t D, double[::1] X, double[:, ::1] o, double[:, :] M, double[:, :] M1, double[:, :] M2):
        cdef:
            tuple results
